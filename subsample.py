...
def get_counts(count_filename, key='id', min_len=None, max_len=None):
    total = 0
    ...
    for r in DictReader(open(count_filename), delimiter='\t'):
        ...
        count_d[r[key]] += int(r['fl_count'])
        total += int(r['fl_count'])
...
def subsample(total, counts, iter=100, min_fl_count=2, step=10**4):
    ...
    for s in sizes:
        ...
        print s, min(tmp), max(tmp), np.mean(tmp), np.std(tmp)
...
if __name__ == "__main__":
    ...
    total, counts = get_counts(args.count_filename, args.by, min_len, max_len)
